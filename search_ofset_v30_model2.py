...
#        self.line_buf = []
    ...
    def edge_predict(self,input):
        ...
        if (self.bf_line_buf[self.tl_point] != 'Null'):
            if (int(self.bf_line_buf[3]) > int(line_buf[3])):
                line_buf[self.tl_point] = str(int(self.bf_line_buf[self.tl_point]) + int(line_buf[3]))
            elif (int(self.bf_line_buf[3]) < int(self.bf_line_buf[self.tl_point])):
                line_buf[self.tl_point] = str(int(self.bf_line_buf[self.tl_point]) + int(line_buf[3]) - int(self.bf_line_buf[3]))
...
#        print("[3]:{0} -> [692]{1}".format(self.line_buf[3],self.line_buf[self.tl_point]))
        ...
        if (self.bf_line_buf[641] == '1' and line_buf[641] == '0'
            ...
            for ssr in range(len(self.sensor_pos)):
                if (int(cutoff_buf[6]) - int(cutoff_buf[5]) - self.sensor_pos[ssr][1] - int(cutoff_buf[4]) >= 0):
                    # Ingestion start position on the Sensor
                    st_pos = int(cutoff_buf[6]) - int(cutoff_buf[5]) - self.sensor_pos[ssr][1] - int(cutoff_buf[4])

                    # Ingestion end position on the Sensor
                    ed_pos = int(cutoff_buf[6]) - int(cutoff_buf[5]) - self.sensor_pos[ssr][1]
                    ...
                    for buf in range(-1,(len(self.pool_data)-1)* -1,-1):
                        if (self.pool_data[buf - 1][3] != 'nan' and self.pool_data[buf][3] != 'nan'):
                            # Search Ingestion end time
                            if ((int(self.pool_data[buf - 1][self.tl_point]) < ed_pos <= int(self.pool_data[buf][self.tl_point])) and edrow_no == 0):
                                edrow_no = buf

                            # Search Ingestion start time
                            if ((int(self.pool_data[buf - 1][self.tl_point]) <= st_pos < int(self.pool_data[buf][self.tl_point])) and strow_no == 0):
                                strow_no = buf - 1
...
#                            print("edrow_no:{0} - strow_no:{1}".format(edrow_no,strow_no))
                    ...
                    for buf in range(-1,(len(self.pool_data)-1)* -1,-1):
                        if (self.pool_data[buf - 1][3] != 'nan' and self.pool_data[buf][3] != 'nan'):
                            ...
                            if (int(self.pool_data[buf - 1][self.tl_point]) < ed_pos <= int(self.pool_data[buf][self.tl_point])):
                                target = True
                                self.output_data = [[]]
                            ...
                            if (int(self.pool_data[buf - 1][self.tl_point]) <= st_pos < int(self.pool_data[buf][self.tl_point])):
                                target = False
